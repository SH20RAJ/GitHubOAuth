...
if (isset($_GET['code'])) {
    ...
    if ($access_token !== false) {
        ...
        $user = $_SESSION['user'] = $github_oauth->getAuthenticatedUser($access_token);
        $loginbutton = 'none';
...
if (isset($user)) {
    ...
    echo '<img src="' . $user->avatar_url . '" alt="Profile Picture">';
    echo '<h2>' . $user->name . '</h2>';
    echo '<p><strong>User ID:</strong> ' . $user->id . '</p>';
    echo '<p><strong>Login:</strong> ' . $user->login . '</p>';
    // Display other user details as needed
    echo '<p><a href="logout.php"><button class="github">Logout</button></a></p>';
    echo '</div>';
} else {
    echo '<div class="login-container">';
    echo '<div class="animated-background"></div>';
    echo '<a href="'.$authorization_url.'">';
    echo '<button class="github">';
